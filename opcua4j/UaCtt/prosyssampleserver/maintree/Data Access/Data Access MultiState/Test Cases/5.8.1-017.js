...
function read581017()
{
    var nodeToBrowse = MonitoredItem.fromSettings( NodeIdSettings.DAStaticMultiStateDiscreteItems() )[0];
    if( AssertNotNullOrEmpty( nodeToBrowse, "No node configured for Browsing." ) == false ) return;
    var referenceDefinitionNode = new MonitoredItem( new UaNodeId( Identifier.MultiStateDiscreteType ), 1 );
    ...
    if( AssertBasicBrowseSuccess( uaStatus, browseResponse, browseRequest ) )
    {
        ...
        for( var resultCounter=0; resultCounter<browseResponse.Results.length; resultCounter++ )
        {
            ...
            AssertNotEqual( false, typeSoughtIsFound, "Node: '" + browseRequest.NodesToBrowse[resultCounter].NodeId + "' " + "' does not derive from Type 'MultiStateDiscreteType')" );
        }
    }
    else
    {
