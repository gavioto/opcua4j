...
function checkHistoryReadError( Request, Response, ExpectedOperationResultsArray, MonitoredItems )
{
    var success = true;
    // check in parameters
    if( arguments.length < 3 )
    {
        addError( "function checkHistoryReadError(): Number of arguments must be 3 or more." );
        return( false );
    }
    ...
    {        
        ...
        for( var i=0; i<Response.Results.length; i++ )
        {
            ...
            if( !bMatch )
            {
                ...
                {
                    addError( "Response.Results[" + i + "].StatusCode = " + dataValue.StatusCode + " but " + ExpectedOperationResultsArray[i].ExpectedResults[0] + " was expected", dataValue.StatusCode );
                    success = false;
                }
            }
