/*  Test 5.9.2 Test 3, prepared by Nathan Pocock; nathan.pocock@opcfoundation.org

    Description:
        Modifies a single monitoredItem setting:
        const TIMESTAMPS = TimestampsToReturn.Server;
        subscription is created and deleted in initialize and cleanup scripts

    Revision History
        Sep-16-2009 NP: Initial version.
        Nov-16-2009 NP: Added verification of timestamps via a Publish() call.
                        REVIEWED.
...
function test592003( timestamps )
{
    ...
    else
    {
        ...
        if ( uaStatus.isGood() )
        {
            ...
            if( AssertEqual( true, publishService.CurrentlyContainsData(), "Expected an initial DataChange but did not receive one." ) )
            {
            ...
            wait( MonitorBasicSubscription.RevisedPublishingInterval );
            publishService.Execute();
            if( AssertEqual( true, publishService.CurrentlyContainsData(), "Expected a DataChange but did not receive one. We previously wrote a value to the item and then modified it to receive just the Server timestamp." ) )
            {
...
function modifyMonitoredItem592003()
{
    test592003( TimestampsToReturn.Server );
}

safelyInvoke( modifyMonitoredItem592003 );